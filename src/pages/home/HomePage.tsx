  ...
  useEffect(() => {
    ...
    const embedReport = async () => {
      try {
        ...
        report = powerbiService.embed(containerRef.current, config) as pbi.Report;
        ...
        report.on('tokenExpired', async () => {
          try {
            const { token: newToken } = await getEmbedConfig(REPORT_NAME);
            await report?.setAccessToken(newToken);
          } catch (err) {
            console.error('Error renovando token Power BI:', err);
          }
        });
